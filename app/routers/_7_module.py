...
from app.models.tables import (Module, User, Subsystem)
...
def create_module(module: schemas.ModuleCreate, session: Session = Depends(get_session), current_user: User = Depends(require_permission("create_modules"))):
    ...
    parent_subsystem = session.get(Subsystem, db_module.subsystem_id)
    try:
        validate_hardware_entity_name(
            session,
            name=db_module.name,
            entity_type="module",
            parent_name=parent_subsystem.name if parent_subsystem else None,
        )
